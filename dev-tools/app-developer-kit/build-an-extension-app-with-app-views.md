...
As the project uses TypeScript, create a simple TypeScript configuration.
...
module.exports = SymADKWebpack(config, package.name);
```
{% endcode %}
...
We are now ready to start building the app. Create a `src` directory and a file named `index.ts` within it.
...
Let's proceed to build the app view itself in a file named `view-a.tsx` within `src/views`.
...
Let's now add a button to the hashtag hover card and a handler to link the context. Use `ADK.buttons.add` to add a new button to the `hashtag` zone that will open the same module using the URL format instead so that we can add an anchor for context.
...
ADK.start({ id: 'adk-example' }).then(() => {
  ...
  ADK.buttons.add('Click Me', 'hashtag', (payload) => {
    ...
    ADK.modules.open('view-a.html#', { title: `ADK View A: #` });
  });
});
